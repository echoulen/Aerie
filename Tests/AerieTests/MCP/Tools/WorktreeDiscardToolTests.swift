...
final class WorktreeDiscardToolTests: XCTestCase {
    ...
    /// Worktree with a dirty working tree. Returns (mainPath, worktreePath).
    private func makeRepoWithDirtyWorktree() throws -> (URL, URL) {
        ...
        try "new".write(to: wt.appendingPathComponent("b.txt"), atomically: true, encoding: .utf8)
        return (main, wt)
    }
    ...
    func test_handle_cleansWorktree() async throws {
        let db = try makeDB()
        let acct = try insertAccount(db)
        let (main, wt) = try makeRepoWithDirtyWorktree()
        ...
        let spy = RefreshSpy()
        let tool = WorktreeDiscardTool(db: db, git: LiveGitService(), refresh: { spy.record($0) })
        ...
        let status = try shell(["git", "-C", wt.path, "status", "--porcelain"])
    ...
    func test_handle_foreignPath_throwsInvalidParams() async throws {
        let db = try makeDB()
        let acct = try insertAccount(db)
        let (main, _) = try makeRepoWithDirtyWorktree()
        let repo = Repository(id: UUID(), name: "R", localPath: main, githubOwner: "o", githubRepo: "r",
                              defaultBranch: "main", primaryAccountId: acct, sortOrder: 0, hidden: false)
        try await db.repos.insert(repo)
        let tool = WorktreeDiscardTool(db: db, git: LiveGitService(), refresh: { _ in })
        do {
    ...
    func test_isWrite_isTrue() throws {
        ...
        XCTAssertEqual(tool.name, "aerie_worktree_discard")
    }
}
