...
// once into a bitmap per size (Core Graphics, no image assets) and parked on
// its home point.
...
final class PlanetFieldView: LayerAnimationView {
    struct Planet {
        ...
        /// Where the planet sits, as a fraction of the view.
        let centre: CGPoint
    ...
    static let planets: [Planet] = [
        Planet(kind: .jupiter, centre: CGPoint(x: 0.88, y: 0.82), diameter: 0.38, opacity: 0.92),
        Planet(kind: .venus, centre: CGPoint(x: 0.09, y: 0.22), diameter: 0.052, opacity: 0.88),
        Planet(kind: .uranus, centre: CGPoint(x: 0.71, y: 0.15), diameter: 0.034, opacity: 0.85),
    ]
    ...
    override func rebuild(size: CGSize, animate: Bool) {
        ...
        for (i, (l, planet)) in zip(planetLayers, Self.planets).enumerated() {
            ...
            l.position = CGPoint(x: planet.centre.x * size.width, y: planet.centre.y * size.height)
        }
    }
}

struct PlanetField: NSViewRepresentable {
    func makeNSView(context: Context) -> PlanetFieldView { PlanetFieldView() }
